...
class ProxyInfo:
	...
	def __init__(self,proxy,failedTimes):
		self.proxy = proxy
	...
	def getFailedTimes(self):
		return self.failedTimes;
	
	def setFailedTimes(self,failedTimes):
		self.failedTimes = failedTimes
		
	def equalTo(self,proxyInfo):
		if proxyInfo is None:
			return False
		if proxyInfo.getProxy()==self.getProxy():
			return True
		else: 
			return False
...
class ProxyManager:
	...
	validProxyList = []
	#lock = threading.Lock()
	THRESHOLD_FAILURE = 70
	
	def isPortAvailable(self,ip,port):
	...
	def checkProxyAvailable(self,ipAndPort):
		...
		if(len(port)>0):
			if self.isPortAvailable(ip,int(port)):
				self.lock.acquire()
				self.validProxyList.append(ProxyInfo(ipAndPort,0))
				self.lock.release()
	
	def __init__(self):
		self.lock = threading.Lock()
		print("[DEBUG] Checking availability of HTTP proxies, please waiting for few minutes...")
		threadAmount = max(len(self.allProxyList)/5,2)
		pool = ThreadPool(threadAmount)
		pool.map(self.checkProxyAvailable,self.allProxyList)
		pool.close() 
		pool.join()
		print("[DEBUG:] all accessible proxy is:"+str(len(self.validProxyList)))
	
	def getRandomProxyInfo(self):
		if len(self.validProxyList)>0:
			proxyInfo = random.choice(self.validProxyList)
			return proxyInfo
		else:
			return None
		
	def getRandomProxy(self):
		proxyInfo = self.getRandomProxyInfo()
		if proxyInfo is not None:
			return proxyInfo.getProxy()
		else:
			return None
	
	def onProxyFailure(self,proxyInfo):
		#self.lock.acquire()
		if proxyInfo is None:
			return
		targetProxyInfos = [proxy for proxy in self.validProxyList if proxy.equalTo(proxyInfo)]
		if len(targetProxyInfos)<1:
			return
		if proxyInfo.getFailedTimes()<=self.THRESHOLD_FAILURE:
			proxyInfo.setFailedTimes(proxyInfo.getFailedTimes()+1)
		else:
			#print "####>>"+proxyInfo.getProxy()
			self.validProxyList.remove(proxyInfo)
			print("[DEBUG:] There are %s valid proxies after removing invalid proxy:%s." %(len(self.validProxyList),proxyInfo.getProxy()))
		#self.lock.release()
		
	def getValidProxyAmount(self):
		return len(self.validProxyList)
